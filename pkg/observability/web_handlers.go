		...
		class AgentaFlowDashboard {
			...
			handleWebSocketMessage(envelope) {
				switch (envelope.type) {
					case 'metrics_update':
						this.handleMetricsUpdate(envelope.data);
						break;
					default:
						console.warn('Unknown WebSocket message type:', envelope.type);
				}
			}
			...
			updateCharts(gpuMetrics) {
				...
				// Guard against empty GPU array to avoid NaN
				if (gpuArray.length === 0) {
					return; // Skip chart update if no GPUs
				}
